...
class FACEDPreprocessing:
    ...
    def bad_channels_interpolate(self, thresh1=None, thresh2=None, proportion=0.3):
        ...
        if Bad_chns.size > 0:
            self.raw.info["bads"].extend(
                [self.montage_index[str(bad)] for bad in Bad_chns]
            )
    ...
    def extract_psd_feature(self, window_size, freq_bands, stft_n=256):
        ...
        for window_index in range(window_num):
            ...
            for band_index, band in enumerate(freq_bands):
                band_ave_psd = self._get_average_psd(
...
def read_data(folderPath):
    # Read the data
    rawdata = mne.io.read_raw_bdf(f"{folderPath}/data.bdf", preload=True)
...
def inter_impedance_inspect(trigger, onset, duration):
    if "Start Impedance" in trigger:
        pos = np.where((trigger == "Start Impedance") | (trigger == "Stop Impedance"))[
            0
        ]
...
def plot_the_topo_corr(y_axis, x_axis, psd_corr, filename):
    ...
    for i, y in enumerate(y_axis):
        for j, x in enumerate(x_axis):
            ...
            vmin = np.min(psd_corr[y][x][0, :])
            vmax = np.max(psd_corr[y][x][0, :])
            print(vmin, vmax)
            im, _ = plot_topomap(
                np.squeeze(psd_corr[y][x][0, :]),
                pos,
                vmin=vmin,
                vmax=vmax,
                axes=axes[i][j],
    ...
    plt.savefig("../Validation/Correlation_analysis/" + filename + ".png")
    plt.clf()
...
def load_srt_raw_newPre(
    data_dir, timeLen, timeStep, fs, channel_norm, time_norm, label_type
):
    n_channs = 30
    n_points = 7500
    data_len = fs * timeLen
    ...
    if channel_norm:
        for i in range(data.shape[0]):
            data[i, :, :] = (data[i, :, :] - np.mean(data[i, :, :], axis=0)) / np.std(
                data[i, :, :], axis=0
            )
    ...
    if time_norm:
        data = (
            data
            - np.tile(np.expand_dims(np.mean(data, axis=2), 2), (1, 1, data.shape[2]))
        ) / np.tile(np.expand_dims(np.std(data, axis=2), 2), (1, 1, data.shape[2]))
    ...
    if isFilt:
        ...
        for i in range(n_subs):
            for vid in range(len(n_samples)):
                data[i, :, int(n_samples_cum[vid]) : int(n_samples_cum[vid + 1])] = (
                    smooth_moving_average(
                        data[
                            i, :, int(n_samples_cum[vid]) : int(n_samples_cum[vid + 1])
                        ],
                        filtLen,
                    )
                )
...
def load_srt_pretrainFeat(features, channel_norm, timeLen, timeStep):
    ...
    if channel_norm:
        for i in range(features.shape[0]):
            features[i, :, :] = (
                features[i, :, :] - np.mean(features[i, :, :], axis=0)
            ) / (np.std(features[i, :, :], axis=0) + 1e-3)
    ...
    for i in range(len(label)):
        label_repeat = label_repeat + [label[i]] * n_samples[i]
    return features, label_repeat, n_samples
    ...
    if filtLen == 1:
        ...
        for i in range(data.shape[1]):
            if i < filtLen // 2:
                ...
                data_smoothed[:, i] = np.mean(
                    data[:, i - filtLen // 2 : i + filtLen // 2], axis=1
                )
